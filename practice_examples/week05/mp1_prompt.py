...
def cost(model: str,usage: Any) -> float :
    if not usage:
        return 0
    return usage.prompt_tokens * RATES[model]["in"] +usage.prompt_tokens * RATES[model]["out"]
...
async def run_one(strategy_name: str, snippet: dict) -> dict:
    ...
    try:
        ...
        raw = response.choice[0].message.content or ""
        ...
        return {
            "strategy" : strategy_name,"snippet_id":snippet["id"],"snippet" : snippet["snippet"],
            "raw_response" : "","extracted" : None,"parse_success" : False,
            "cost_usd" :0.0,"latency_s": time.perf_counter() - started, "error": type(exc).__name__ + ": " + str(exc),
        }
...
async def score_judge_all(results : list[dict[str,Any]],golden : dict[str,dict[str,Any]]) -> None:
...
def build_summary(scored: list[dict]) -> pd.DataFrame:
    ...
    # Include extraction and judge costs.
    df["total_cost_usd"] = (
        df["cost_usd"] + df["judge_cost_usd"]
    )
...
def write_reports(table: pd.DataFrame,results: dist[str,Any]) -> None:
    ...
    j10 = [item for item in results if item["snippet"] == "j10" ]
    j10_notes = "\n".join(
        "- {} : years ={},accuracy = {}/3,judge ={}/4.".format(
            item["strategy"],item["extracted"],
            item["years_experience_required"] if item["extracted"] else None,
...
async def main() -> None:
        global client,call_gate,snippets,judge_gate
        ...
        await score_judge_all(scored,golden)
